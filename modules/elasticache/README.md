...
The module keeps secure defaults for new deployments: at-rest encryption is
enabled and transit encryption is required. Listener V2 releases that do not
yet support TLS can use the following temporary shadow-mode compatibility
configuration:
...
Keep network access restricted to the workload subnets or explicitly supplied
CIDRs. The compatibility configuration is not intended for promotion to the
primary listener.

When using `cluster_name` discovery, note that EKS only reports subnets in the
cluster VPC configuration. Supply the CIDRs of any additional node or pod
subnets explicitly so their workloads can reach ElastiCache.

The documented node types are sizing starting points rather than automatic
testnet/mainnet profiles. Validate the choice against event rate, payload size,
maximum consumer backlog, and latency requirements.
...
| Name | Description |
|------|-------------|
| <a name="output_connection_scheme"></a> [connection\_scheme](#output\_connection\_scheme) | Connection URI scheme: rediss when transit encryption is enabled, otherwise redis. |
| <a name="output_k8s_service_fqdns"></a> [k8s\_service\_fqdns](#output\_k8s\_service\_fqdns) | In-cluster DNS names of the ExternalName Services, keyed by namespace. Empty map when elasticache.k8s\_service.enabled = false. |
